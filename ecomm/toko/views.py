...
class HomeListView(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['all_kategori_url'] = reverse('toko:home-produk-list')
        context['shirt_kategori_url'] = reverse('toko:home-produk-list') + '?kategori=S'
        context['outware_kategori_url'] = reverse('toko:home-produk-list') + '?kategori=OW'
        context['sportware_kategori_url'] = reverse('toko:home-produk-list') + '?kategori=SW'
        return context
    ...
    def get_queryset(self):
        ...
        if min_price and max_price:
            queryset = queryset.filter(harga__range=(min_price, max_price))
        elif min_price:
            queryset = queryset.filter(harga__gte=min_price)
        elif max_price:
            queryset = queryset.filter(harga__lte=max_price)
        elif q:
            queryset = queryset.filter(nama_produk__icontains=q)
...
class ProductDetailView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        print("999 kategori", self.object.kategori, self.object.pk)
        similar_products = ProdukItem.objects.filter(kategori=self.object.kategori)
        print("cek", similar_products)
